...
static GMLCoreDispatcher_IOS * ins = nil;

@implementation GMLCoreDispatcher_IOS
...
-(void)addEventListener:(NSString *)eventName execFunc:(void (^)(GMLEvent_IOS *))_execFunc
{
    if(eventDic == nil)
    {
...
-(void)dispatchEvent:(GMLEvent_IOS *)e{
    if(eventDic == nil)
    {
        return;
    }
    ...
    for (NSString* key in eventDic.allKeys) {
        ...
        {
            //执行代码块
            ((void (^)(GMLEvent_IOS *))[eventDic valueForKey:key])(e);
            break;
        }
    }
}
